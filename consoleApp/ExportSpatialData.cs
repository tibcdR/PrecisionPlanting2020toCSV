...
{
    public class ExportSpatialData
    {
        // public static void ExportSpatialDatatoCSV(Catalog catalog, LoggedData loggedData)
        // {
        // }
        ...
        private void CreateRow(Dictionary<int, IEnumerable<WorkingData>> workingDataDictionary, SpatialRecord spatialRecord)
        {
            ...
            if (spatialRecord.Geometry != null)
            {
                ...
                dataRow["Latitude"] = (spatialRecord.Geometry as Point).Y.ToString().Replace(',', '.'); //Y
                dataRow["Longitude"] = (spatialRecord.Geometry as Point).X.ToString().Replace(',', '.'); //X
                dataRow["Elevation"] = (spatialRecord.Geometry as Point).Z.ToString().Replace(',', '.'); //Z
            }
